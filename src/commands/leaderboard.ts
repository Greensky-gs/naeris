...
export default new AmethystCommand({
    ...
    await Promise.all([ interaction.deferReply().catch(log4js.trace),
    ...
    const data = leaderboard.map(x => ({
        user: {
            ...
            pp: interaction.guild.members.cache.get(x.user_id)?.user?.displayAvatarURL({ forceStatic: true, extension: 'jpg', size: 128 }) ?? defaultPP
        },
        coins: {
            coins: x.coins,
            bank: x.bank,
