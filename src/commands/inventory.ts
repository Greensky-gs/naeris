...
export default new AmethystCommand({
    ...
    if (!inventory.items.length)
        return interaction.reply(`${pnjTalk('gerard')} Vous ne m'avez rien acheté`).catch(log4js.trace);
    ...
    await Promise.all([interaction.deferReply(), interaction.guild.roles.fetch()]).catch(log4js.trace);

    const data = (items: inventoryItem[]) => ({
        guild: interaction.guild,
        user: {
            username: (interaction.member as GuildMember)?.nickname ?? interaction.user.username,
            pp: interaction.user.displayAvatarURL({ forceStatic: true, extension: 'jpg', size: 128 })
        },
        items
    });
