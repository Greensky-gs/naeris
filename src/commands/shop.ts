...
export default new AmethystCommand({
    ...
    if (cmd === 'acheter') {
        ...
        if (item.remaining === 0 && item.quantity !== 0) return interaction.reply(`${pnjTalk('gerard')} Désolé, il ne m'en reste plus en stock`).catch(log4js.trace)

        if (item.price > stats.coins) return interaction.reply(`${pnjTalk('gerard')} Tu n'as pas assez de gold pour acheter ça`).catch(log4js.trace)

        if (item.type === 'role' && shop.getInventory(interaction, interaction).items.find(x => x.id === item.id)) return interaction.reply(`${pnjTalk('gerard')} Vous avez déjà ce rôle`).catch(log4js.trace)

        const confirmation = await confirm({
            interaction,
            user: interaction.user,
            content: { content: `${pnjTalk("gerard")} Êtes-vous sûr d'acheter **${item.name}** pour **${numerize(item.price)}** gold ?` }
        }).catch(log4js.trace)
