...
export default new AutocompleteListener({
    ...
    run: ({ focusedValue, interaction }) => {
        ...
        const selected = list.filter(
            (x) =>
                x.name.toLowerCase().includes(focusedValue) ||
                focusedValue.includes(x.name.toLowerCase()) ||
                x.content.toLowerCase().includes(focusedValue) ||
                focusedValue.includes(x.content.toLowerCase())
        );

        return selected.map((x) => ({
            name: resize(`${x.name} - ${x.type === 'role' ? 'Rôle' : 'Texte'}`),
            value: x.id.toString()
        }));
    }
});
