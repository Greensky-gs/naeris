...
export default new AmethystEvent('commandDenied', (command, reason) => {
    if (command.interaction.isChatInputCommand()) {
        ...
        if (reason.code === commandDeniedCode.UserMissingPerms || reason.code === commandDeniedCode.ClientMissingPerms) {
            command.interaction.reply({
                embeds: [ missingPermissions(command.interaction, reason.metadata.permissions.missing, reason.code === commandDeniedCode.ClientMissingPerms ? 'client' : 'user') ],
                ephemeral: true
            }).catch(log4js.trace)
        }
