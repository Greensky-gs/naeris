...
export default new ButtonHandler({
    ...
    const embed = () => {
        return baseValid(user)
            ...
            .setDescription(
                `Vous modifiez l'item **${item.name}**\nFin de la modification <t:${Math.floor(
                    endsAt / 1000
        ...
        return [
            row(
                ...
                buttonBuilder({
                    label:
                        item.quantity === 0
                            ? 'Quantité restante (non modifiable si quantité infinie'
                            : 'Quantité restante',
                    disabled: item.quantity === 0 || disabled,
                    style: 'Secondary',
                    id: 'remaining'
            ...
            const vocabulary = {
                ...
                remaining: {
                    modal: {
                        ...
                        label: ' Quantité',
                        placeholder: "entrez le nombre d'items achetables"
                    }
                }
            }[id];
        ...
        if (id === 'name' || id === 'content') {
            const vocabulary = {
                name: {
                    ...
                    max: 80,
                    modal: { title: 'Nom', label: "nom de l'item", placeholder: "entrez le nom de l'item" }
                },
                content: {
                    style: TextInputStyle.Paragraph,
                    max: 250,
                    modal: { title: 'Contenu', label: "Contenu de l'item", placeholder: "entrez le contenu de l'item" }
                }
            }[id];
