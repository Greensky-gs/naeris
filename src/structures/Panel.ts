...
export class Panel {
    ...
    private async start() {
        this.channel = (this.client.channels.cache.get(process.env.panel) ??
            (await this.client.channels.fetch(process.env.panel).catch(log4js.trace))) as TextChannel;
        if (!this.channel) {
        ...
        const msg = await this.channel
            .send({
                embeds: [
                    base(this.client.user)
                        .setColor('#8177E5')
                        .setTitle('Panel de contrôle')
                        .setDescription(`Panel de contrôle de ${pingUser(this.client.user)}`)
                ],
